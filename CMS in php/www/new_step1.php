...
$query  = "insert into step1 (";
$query .= "menu_name, position,pages_id,visible,content) ";
$query .= "values (";
$query .= " '{$menu_name}', {$position}, {$pages_id},{$visible}, '{$content}'";
$query .= ");";
...
$result = mysqli_query($connection, $query);
        ...
        if ($result && mysqli_affected_rows($connection) >= 1 ){
			...
			redirect_to("manage_content.php?pages={$pages_id}");
			} else {
			$message = "page update failed here at this place";
			redirect_to("new_step1.php");
...
<div id="main">
	...
	<h2>Create page </h2>
		<form action ="new_step1.php?pages=<?php echo urlencode($current_pages["id"]); ?>" method="post">
			<p> Page name : 
			...
			<p> Position :
				...
				<?php
				$step1_set =find_step1_for_pages($current_pages["id"]);
				$step1_count = mysqli_num_rows($step1_set);
				for ($count=1;$count <=($step1_count +1) ;$count++) {
		...
		<a href="manage_content.php?pages=<?php echo urlencode($current_pages["id"]); ?>">Cancel </a>
		&nbsp;
		&nbsp;
		&nbsp;
		<a href="delete_step1.php?step1=<?php echo urlencode($current_step1["id"]); ?>" onclick = "return confirm('Are you sure ? you want to delete <?php echo  $current_step1["menu_name"]; ?>');"> Delete page</a>
		&nbsp;
		&nbsp;
		&nbsp;
