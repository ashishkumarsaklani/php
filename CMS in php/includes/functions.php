...
function public_navigation2($subjects_array,$pages_array,$step1_array,$step2_array) {
		...
		$output .= "<li";
			...
			{   $output .= " class=\"selected\" ";}
				...
				$output .= urlencode($subjects["id"]); 
				$output .= "\">";
				$output .= "<img class=\"img-circle\"; src=\"/image1.php?tab=subjects&r=menu_pic&id=";
				$output .= htmlentities($subjects["id"]) ;
				$output .="\" style=\"width:95%;height:8%;resize:both;\">";
				$output .= "</a>";
			// accordian effect
				if ($subjects_array["id"] == $subjects["id"] || $pages_array["subjects_id"] == $subjects["id"]) {
					$pages_set = find_pages_for_subjects($subjects["id"]) ;
...
function find_step2_by_id($step2_id,$public =true,$column="*"){
		global $connection ;
...
function find_row($table,$condition){
	global $connection ;
		...
		$table_set = mysqli_query($connection,$query);
		confirm_query($table_set);
		$count= mysqli_num_rows( $table_set );
...
function find_properties_for_elements_name($elements_name) {
	
	$elements_name=trim($elements_name);
	global $connection;
	$safe_elements_name=mysqli_real_escape_string($connection,$elements_name);
		$query  = "select * ";
		$query .= "from elements ";
		$query .= "where menu_name='{$safe_elements_name}' ";
		...
		$query  = "select * ";
		$query .= "from `properties` ";
		$query .= "where elements_id={$elements["id"]} ";
...
function create_step2_table($menu_name){
						global $connection ;
		
						$query  ="	CREATE TABLE `{$menu_name}` (";
						$query .= "	id INT(6) UNSIGNED AUTO_INCREMENT PRIMARY KEY,";
						$query .= "	step2_id int(11) NOT NULL,";
						$query .= "	menu_name VARCHAR(30) NOT NULL,";
						$query .= "	content TEXT,";
						$query .= " place float(4),";	
						$query .= "	height  VARCHAR(12),";
						$query .= "	width  VARCHAR(12),";
						$query .= "	position VARCHAR(12),";
						$query .= "	`float`  VARCHAR(12),";
						$query .= "	`clear`  VARCHAR(12),";
						$query .= "	color VARCHAR(12),";
						$query .= "	border VARCHAR(12),";
						$query .= "	`border-bottom` VARCHAR(12),";
						$query .= "	`border-top` VARCHAR(12),";
						$query .= "	`border-left` VARCHAR(12),";
						$query .= "	`border-right` VARCHAR(12),";
						$query .= "	padding VARCHAR(12),";
						$query .= "	`padding-top` VARCHAR(12),";
						$query .= "	`padding-bottom` VARCHAR(12),";
						$query .= "	`padding-left`  VARCHAR(12),";
						$query .= "	`padding-right` VARCHAR(12),";
						$query .= "	margin VARCHAR(12),";
						$query .= "	`margin-left` VARCHAR(12),";
						$query .= "	`margin-right` VARCHAR(12),";
						$query .= "	`margin-bottom` VARCHAR(12),";
						$query .= "	`margin-top` VARCHAR(12),";
						$query .= "	`background-color` VARCHAR(12),";
						$query .= "	display VARCHAR(12),";
						$query .= "	`font-size` VARCHAR(12),";
						$query .= "	`line-height` VARCHAR(12),";
						$query .= "	`list-style` VARCHAR(12),";
						$query .= "	overflow VARCHAR(12),";
						$query .= "	resize VARCHAR(12),";
						$query .= "	size VARCHAR(12),";
						$query .= "	`text-decoration` VARCHAR(12),";
						$query .= "	`text-align` VARCHAR(12),";
						$query .= "	`text-overflow` VARCHAR(12),";
						$query .= "	visibility  VARCHAR(12),";
						$query .= "	`z-index`  VARCHAR(12),";
						$query .= "	image BLOB";
						$query .= "	)";
...
function get_otp($telephone){
$num_length = strlen((string)$telephone);
if($num_length == 10) {
    // Pass
			...
			$table=check_table("temp_otp");			
			if ($table == FALSE ){
						
						
						$query  = "	CREATE TABLE `temp_otp` (";
						$query .= "	id INT(6) AUTO_INCREMENT PRIMARY KEY,";
						$query .= "	telephone bigint(10) unique NOT NULL,";
						$query .= "	otp varchar(6) NOT NULL,";
						$query .= "	expire bigint(10)";
						$query .= "	)";
					...
					$record=find_row($tabl,$cond);
					$otp = strtoupper(substr(uniqid(mt_rand(),true),6,6));
					$expire= my_time();
					$expire = strtotime("+15 min",$expire);
					if ($record == false ){
						$query  = "insert into `temp_otp` (  `otp` ,`telephone` , `expire` ) values ( '{$otp}',$telephone,$expire );";
						}
					else{
						$query  = "update `temp_otp`  set `otp` ='{$otp}' , `expire` =$expire where `telephone` = $telephone limit 1 ; ";
						}								
						$result = mysqli_query($connection, $query);
...
function check_otp($telephone,$otp){
			$c_t=my_time();
			$tabl="temp_otp";
			$cond="`telephone` = $telephone";
			$record=find_row($tabl,$cond);
			...
			if(!isset($rec_exp)){$rec_exp=null;}
			if ($rec_exp==null){	
				
				$mes = "Please generate Otp";
			}elseif($rec_otp != $otp ){
				$mes = "Otp not valid";
			}elseif($rec_exp < $c_t ){
				$mes = "Otp is expired";
